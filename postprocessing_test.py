...
from full_metrics_pipeline import _fidelity_curve
...
OUTPUT_PATH_DELETION = os.path.join(OUTPUT_DIR, f"paper_style_deletion_{IMG_NUM}_{DELETION_BASELINE}.png")
OUTPUT_PATH_OVERLAY = os.path.join(OUTPUT_DIR, f"gradcam_overlay_{IMG_NUM}.png")
...
# This is the number of curve steps used by _fidelity_curve.
# In your pipeline this corresponds to fidelity_features_per_step,
# but the function actually treats it as the number of steps.
DELETION_STEPS = 300

# Paper-style snapshots
...
def parse_model_filename(model_path: str) -> Optional[dict]:
    ...
    and:
        ...
        finetuned_fidelity_k=[5,9,11]_64_3_64_1_wd0.0_do0.0.pth
    """
...
def generate_gradcam_using_project_code(model: torch.nn.Module, img: torch.Tensor, device: torch.device):
    """
    Uses the project GradCAM implementation from shared_code.py.
    """
...
def build_pipeline_saliency_from_gradcam(heatmap: np.ndarray, image: torch.Tensor):
    """
    Your full_metrics_pipeline GradCAM saliency is shaped as (C,H,W),
    with the same GradCAM heatmap broadcast into every input channel.

    This function creates the same format so _fidelity_curve receives the
    same kind of ranked input as in the real pipeline.
    """
    C = image.shape[0]
...
def compute_project_deletion_curve(model, image, saliency, label, device):
    """
    Uses your own metric implementation:
...
def gaussian_blur_baseline(
    ...
    """
    Creates a blurred version of the original image.
    This is used as the deletion baseline instead of black pixels.
    """
    if kernel_size % 2 == 0:
        kernel_size += 1
...
def make_masked_image_like_project_metric(image, ranked, fraction_deleted, device):
    """
    Recreates the deletion behavior from full_metrics_pipeline._fidelity_curve,
    but uses the selected deletion baseline: black, blur, or mean.

    Important:
    This still follows your current flattened C*H*W feature deletion order,
    so it stays visually consistent with the current project metric.
    """
    image = image.to(device)
...
#     Important:
#     This deletes flattened C*H*W feature entries, because that is what your
#     current metric code does.
#     """
#     C, H, W = image.shape
#     n_features = C * H * W
...
def save_gradcam_overlay_plot(img, heatmap, pred_label, pred_conf, tracked_label, hp):
    ...
    im = axes[1].imshow(heatmap, cmap=GRADCAM_CMAP, vmin=0, vmax=1)
    ...
    print(f"Saved overlay plot to: {OUTPUT_PATH_OVERLAY}")
...
def save_paper_style_deletion_plot(
    ...
    # Explanation panel
    axes[0].imshow(heatmap, cmap="viridis", vmin=0, vmax=1)
    axes[0].set_title("Explanation")
    axes[0].axis("off")

    # Deletion steps: 0%, 25%, 50%, 75%, 100%
    ...
    fig.suptitle(
        f"Project GradCAM + project deletion metric | Tracked class: {tracked_name}\n"
        f"k={hp['kernel_size']}, cf={hp['conv_filter']}, cl={hp['conv_layer']}, "
        f"dn={hp['dense_neuron']}, dl={hp['dense_layer']}, do={hp['dropout']}",
        fontsize=11,
    )
    ...
    print(f"Saved deletion plot to: {OUTPUT_PATH_DELETION}")
...
def main():
    ...
    print("Generating GradCAM using shared_code.GradCAM...")
    heatmap = generate_gradcam_using_project_code(
        model=model,
        img=image,
        device=device,
    )

    print("Saving GradCAM overlay plot...")
    save_gradcam_overlay_plot(
        img=image,
        heatmap=heatmap,
    ...
    print("Building pipeline-style saliency...")
    saliency = build_pipeline_saliency_from_gradcam(
        heatmap=heatmap,
        image=image,
    )
    ...
    print("Saving paper-style deletion plot...")
