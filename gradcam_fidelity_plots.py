...
def build_pipeline_saliency_from_gradcam(heatmap: np.ndarray, image: torch.Tensor):
    """
    Broadcasts the GradCAM heatmap into every input channel,
    matching the (C, H, W) saliency format used by the pipeline.
    """
    C = image.shape[0]
...
def save_plot(
    ...
    # --- GradCAM heatmap ---
    axes[0].imshow(heatmap, cmap=GRADCAM_CMAP, vmin=0, vmax=1)
...
def process_image(model, hp, img_num, target_label, device):
    ...
    print(f"  Initial tracked-class probability: "
          f"{model_prob_for_label(model, image, target_label):.4f}")

    print("  Generating GradCAM...")
    heatmap = generate_gradcam(model, image, device)

    saliency = build_pipeline_saliency_from_gradcam(heatmap, image)

    print("  Computing deletion/insertion curves...")
    ...
    out_path = os.path.join(
        OUTPUT_DIR,
        class_name.lower(),
        f"GRAD_{class_name.lower()}_{img_num}_{DELETION_BASELINE}.png",
    )
...
def main():
    ...
    for i, (img_num, target_label) in enumerate(work, 1):
        print(f"\n--- [{i}/{len(work)}] ---")
        process_image(model, hp, img_num, target_label, device)
