...
# In your ImageFolder setup this should be:
CLASS_0_NAME = "Cat"
CLASS_1_NAME = "Dog"
...
def plot_histogram(
    ...
    plt.axvline(mean_p_dog, linestyle="--", linewidth=2, label=f"mean p({CLASS_1_NAME}) = {mean_p_dog:.3f}")
    plt.axvline(0.5, linestyle=":", linewidth=2, label="neutral = 0.5")

    plt.title(
        title
        + "\n"
        + f"mean p({CLASS_0_NAME})={mean_p_cat:.3f}, "
        + f"mean p({CLASS_1_NAME})={mean_p_dog:.3f} → {describe_bias(mean_p_dog)}"
    )
    plt.xlabel(xlabel)
...
def plot_overlay(
    ...
    if len(blur) > 0:
        ...
        plt.hist(
            ...
            alpha=0.55,
            edgecolor="black",
            label=f"blur: mean p({CLASS_1_NAME})={blur_mean:.3f}, {describe_bias(blur_mean)}",
        )
        plt.axvline(blur_mean, linestyle="--", linewidth=2)
    ...
    if len(mean) > 0:
        ...
        plt.hist(
            ...
            alpha=0.55,
            edgecolor="black",
            label=f"mean: mean p({CLASS_1_NAME})={mean_mean:.3f}, {describe_bias(mean_mean)}",
        )
        plt.axvline(mean_mean, linestyle="--", linewidth=2)

    plt.axvline(0.5, linestyle=":", linewidth=2, label="neutral = 0.5")

    plt.title(f"{model}\nOriginal images: {class_name} | blur vs mean baseline")
    plt.xlabel(xlabel)
    plt.ylabel("Number of images")
    plt.xlim(0, 1)
    plt.xticks(np.arange(0, 1.01, 0.05), rotation=45)
    ...
    for model, model_df in df.groupby("model"):
        ...
        if MAKE_CLASS_PLOTS:
            for class_name, class_df in model_df.groupby("original_class_name"):
                ...
                for baseline, sub in class_df.groupby("baseline"):
                    ...
                    title = (
                        f"{model}\n"
                        f"Original images: {class_name} | baseline: {baseline}\n"
                        f"n={len(values)}, std p({CLASS_1_NAME})={values.std(ddof=1):.3f}"
                    )
                    plot_histogram(values, title, out_path, xlabel=xlabel)
                ...
                if MAKE_OVERLAY_PLOTS:
                    ...
                    plot_overlay(overlay_df, model, str(class_name), out_path, xlabel=xlabel)
                    print(f"Saved: {out_path}")
        ...
        if MAKE_ALL_PLOTS:
            for baseline, sub in model_df.groupby("baseline"):
                ...
                title = (
                    f"{model}\n"
                    f"Original images: ALL Cat + Dog | baseline: {baseline}\n"
                    f"n={len(values)}, std p({CLASS_1_NAME})={values.std(ddof=1):.3f}"
                )
                plot_histogram(values, title, out_path, xlabel=xlabel)
                print(f"Saved: {out_path}")
            ...
            if MAKE_OVERLAY_PLOTS:
                ...
                plot_overlay(overlay_df, model, "ALL Cat + Dog", out_path, xlabel=xlabel)
                print(f"Saved: {out_path}")
